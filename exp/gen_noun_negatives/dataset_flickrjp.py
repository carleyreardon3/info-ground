...
class FlickrJPDatasetConstants(Constants):
    def __init__(self,subset):
        ...
        self.noun_tokens_json = os.path.join(
            flickr_jp_paths['proc_dir'],
            flickr_jp_paths['noun_tokens'][subset])
        self.noun_vocab_json = os.path.join(
            flickr_jp_paths['proc_dir'],
            flickr_jp_paths['noun_vocab'][subset])
...
class FlickrJPDataset(Dataset):
    ...
    def get_caption(self,image_id,cap_num):
        try:
            cap = self.sentences[image_id][cap_num]['sentence']
        except:
            print(len(self.image_ids), image_id, cap_num)
        else:
            return cap
    ...
    def __getitem__(self,i):
        ...
        if self.const.read_noun_token_ids is True:
            if self.noun_token_ids[i] is None:
