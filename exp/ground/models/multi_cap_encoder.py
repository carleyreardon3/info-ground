...
class MultiCapEncoderConstants(io.JsonSerializableClass):
    def __init__(self):
        super().__init__()
...
class MultiCapEncoder(nn.Module,io.WritableToFile):
    ...
    def select_embed(self,embed,token_ids):
        ...
        max_tokens = token_ids.size(dim=1)
        B,T,D = embed.size()
...
if __name__=='__main__':
    const = MultiCapEncoderConstants()
    const.output_attentions = True
    cap_encoder = MultiCapEncoder(const)
