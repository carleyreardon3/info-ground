...
class FlickrJPDatasetConstants(Constants):
    def __init__(self,subset):
        super().__init__()
        self.subset = subset
        self.det_dir = os.path.join(flickr_jp_paths['det_dir'],self.subset)
        self.image_dir = flickr_jp_paths['image_dir']

        self.image_ids_txt = os.path.join(
            flickr_jp_paths['downloads_dir'],
            flickr_jp_paths['subsets'][self.subset])
        self.phrase_boxes_json = os.path.join(
            flickr_jp_paths['proc_dir'],
            flickr_jp_paths['phrase_boxes'][self.subset])
        self.sentences_json = os.path.join(
            flickr_jp_paths['proc_dir'],
            flickr_jp_paths['sentences'][self.subset])
        ...
        self.noun_adj_tokens_json = os.path.join(
            flickr_jp_paths['proc_dir'],
            flickr_jp_paths['noun_adj_tokens'][subset])
        ...
        self.neg_noun_samples_json = os.path.join(
            flickr_jp_paths['proc_dir'],
            flickr_jp_paths['noun_negatives']['samples'][subset])
        self.neg_noun_samples_h5py = os.path.join(
            flickr_jp_paths['local_proc_dir'],
            flickr_jp_paths['noun_negatives']['feats'][subset])
        self.read_neg_noun_samples = True
        self.num_neg_nouns = 25
        self.neg_noun_feat_dim = 768


class FlickrJPDataset(Dataset):
    ...
    def get_caption(self,image_id,cap_num):
        x = self.sentences[image_id]
        y = x[cap_num]['sentence']
        return y
    ...
    def __getitem__(self,i):
        ...
        if self.const.read_noun_adj_tokens is True:
            if self.noun_adj_token_ids[i] is None:
                to_return['noun_adj_token_ids'] = []
            else:
...
if __name__=='__main__':
    const = FlickrJPDatasetConstants('val')
    dataset = FlickrJPDataset(const)
